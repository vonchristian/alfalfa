  ...
  class OfficeExpensesController < ApplicationController
    layout 'accounting'
        ...
        def create
          ...
          authorize @entry
          if @entry.save
            @entry.entriable_id = @entry.recipient_id
            @entry.entriable_type = @entry.recipient_type
            redirect_to accounting_expenses_path, notice: "Office expense saved successfully."
          else
            render :new
          end
        end
        ...
        private
        def entry_params
          params.require(:accounting_entry).permit(:description, :entry_type, :recipient_id, :recipient_type,  :date, :reference_number, :entriable_id, :entriable_type, :credit_amounts_attributes=> [:id, :amount, :account],:debit_amounts_attributes=> [:id, :amount, :account])
        end
      end
    end
