class OrderLineItemsPdf < Prawn::Document
  ...
  def table_data
    if @inventory_id.present?
      ...
      @table_data ||= @line_items.where(:inventory_id => @inventory_id).all.map { |e| [ e.date.strftime("%B %e, %Y"), "#{e.order.try(:customer, :name) }" + " - #{e.order.purpose }", e.inventory.try(:name), e.inventory.unit, e.quantity, price(e.unit_cost), price(e.total_cost) ] } +
      @table_data ||= [["", "", "", "", "", "TOTAL", "#{price(total_orders)}"]]
    else
      [["Date", "Issued To", "Item", "Unit", "Quantity", "Unit Cost", "Total Cost"]] +
      @table_data ||= @line_items.map { |e| [ e.date.strftime("%B %e, %Y"), "#{e.order.try(:customer, :name) }" + " - #{e.order.purpose }", e.inventory.try(:name), e.inventory.unit, e.quantity, price(e.unit_cost), price(e.total_cost) ] } +
      @table_data ||= [["", "", "", "", "", "TOTAL", "#{price(total_orders)}"]]
    end
  end
end
