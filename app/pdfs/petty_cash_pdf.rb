class PettyCashPdf < Prawn::Document
  ...
  def previous_credit_balance
    if Accounting::Entry.any?
      Account.find_by_name("Petty Cash").credits_balance({from_date: oldest?, to_date: @from_date.yesterday.end_of_day})
    end
  end
  ...
  def starting_balance
    if previous_debit_balance.present? && previous_credit_balance.present?
      if previous_debit_balance - previous_credit_balance <= 0
        0
      elsif previous_debit_balance - previous_credit_balance > 0
